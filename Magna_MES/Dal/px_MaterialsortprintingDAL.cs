    ...
    public class GetModel
    {
        ...
        public List<GetSP> GetSPModels(int Skip, int Take, string csh = "", params DateTime?[] time)
        {
           ...
           ////db.Database.ExecuteSqlCommand("update mg_PartOrder set OrderIsPrintSYS='0' where OrderIsPrintSYS is null ");
            ...
            string sql = "select * from View_px_AllList where  订单号 in(select distinct  订单号  from   View_px_AllList) ";
            if ((csh != null && csh.Length > 1) || (time != null && time.Length > 0))
                sql = "select * from View_px_AllListALLLLL where  订单号 in(select distinct  订单号  from   View_px_AllList) ";
            strSql.Append(sql);
            if (time != null && time.Length > 0)
            {
            ...
            else
            {
                ...
                int num = 0;
                string total=Listcount.Count.ToString();
                if (Skip <= 0)  Skip = 0;
                if (Listcount.Count < Skip + Take) num = Listcount.Count;
                else num = Skip + Take;
...
//            }
            ...
            //var modellist = (List<GetSP>)getmodel.GetSPModels(time).Skip((Page - 1) * PageSize).Take(PageSize).ToList();
               ...
               total = modellist.Count.ToString();
